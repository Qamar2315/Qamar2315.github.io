    ...
    function populateModal(item) {
        ...
        if (item.long_description) {
            ...
            descriptionHTML = `
                ...
                <p class="mb-4">${item.problem_statement ? marked.parseInline(item.problem_statement) : ''}</p>
                <h4 class="font-semibold text-slate-800">My Solution</h4>
                <p>${item.solution_delivered ? marked.parseInline(item.solution_delivered) : ''}</p>
            `;
        }
        ...
        if (item.client_feedback) {
            ...
            modalFeedbackQuote.innerHTML = `"${item.client_feedback.quote ? marked.parseInline(item.client_feedback.quote) : ''}"`;
            modalFeedbackClient.innerHTML = `- ${item.client_feedback.client_name ? marked.parseInline(item.client_feedback.client_name) : ''}`;
        }
        ...
        if (item.key_challenges && item.key_challenges.length > 0) {
            ...
            modalChallengesList.innerHTML = item.key_challenges.map(c => `
                <div class="p-4 bg-slate-50 rounded-lg border border-slate-200">
                    <div class="font-semibold text-slate-700 mb-1">${c.challenge ? marked.parseInline(c.challenge) : ''}</div>
                    <div class="text-slate-600">${c.solution ? marked.parseInline(c.solution) : ''}</div>
                </div>
            `).join('');
        }
